...
def erode_grow(dadt, y, dydt, NG, NDust, dust_calc, dTime, dust_t):
    ...
    for i in prange(NDust):
        ...
        if dydt[NG + (i*N_MOMENTS +1)] != 0.0:
            prnt(dust_t[i][15])
            new_grn_sz = dust_t[i][15] * dust_calc[i][-1]**(onethird) ## -1 is ncrit #dust_calc[i].Js * dust_calc[i].ncrit ** (1/3)
        ...
        for sizeIDX in list(range(numBins-1)):
            ...
            if y[start + (i*numBins) + sizeIDX] != 0.0:
                prnt('dt: '+str(dadt[i*numBins + sizeIDX])+', shrink '+str(shrink)+', grow '+str(grow)+', grain size '+str(grn_size)+', bin value '+str(y[start + (i*numBins) + sizeIDX]))
