...
from .compatibility import numba, is_jitted
...
class Solver(ABC, metaclass=MetaSolver):
    ...
    # user rhs, same as rhs if originally jitted and with the right signature
    user_rhs: Callable

    # extra args for the callable
    params: np.ndarray or None

    # las LEN_HISTORY times, states, dervs
    cache: AlignedBuffer

    # class methods that build steps
    _fixed_step_builder: Callable
    _step_builder: Callable

    # which interpolator; None->self._interpolate
    _interpolator = None
